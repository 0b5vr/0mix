...
<small>▲ [Emix by Epoch](https://www.pouet.net/prod.php?which=66066)</small>
...
<small>▲ [Clean Slate by Conspiracy](https://www.pouet.net/prod.php?which=88524)。Windows 64K Intro。数多くのジオメトリが美麗なレンダリングで表示される</small>
...
<small>▲ Yan Cookによる、Wavetable SynthからTechnoを生み出すチュートリアル</small>
...
<small>▲ STOORによる7時間弱のTechno Machine Live</small>
...
<small>▲ Elektron Syntaktのデモ動画</small>
...
<small>▲ Yebisu303氏によるDirtywave M8を用いたHouseのパフォーマンス</small>

どっちも、触って「ぼくはGLSLでいいかな」となった。マジの異常者。
...
実際、2500回のforループで大量のサイン波を生み出すコードが存在する。
...
<small>▲ [Wackelkontakt by Alcatraz & Prismbeings](https://www.pouet.net/prod.php?which=85220)。Oidosによるパッドサウンドが綺麗な4k Intro</small>
...
本来は、Perlin NoiseのFBMテクスチャを使う予定であったが、パフォーマンスのわかりやすさを優先し、テクスチャやサンプルは一切使わない構成とした。
...
<small>▲ Overdrive 2 by Titan。MegadriveによるDemo</small>
...
<small>▲ DARK TRAIN - S-Bahn。beatmaniaIIDX 28 BISTROVERで収録された、ダークなTechno</small>
...
田所先生の、事前にコメントアウトされた状態でパターンをたくさん持っておく体裁を参考にした。

サンプルベースのライブコーディングDSLを定義する択もあるとは思うが、今回はGLSLを用いた。
理由のひとつは、コード上で行える一つ一つの操作がモジュラーシンセと同程度に低レベルで、個人的にはわかりやすいこと。
...
また、楽曲の雰囲気に合わせて、Post ProcessingのColor Gradingを調整する。
